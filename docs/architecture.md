...
- `wilderness`：通过 `archetype` 选择河谷、裂谷、山地、冰原、遗迹、地下湖、森林或沼泽，以地形平台、水体、桥、植被和坡道构成战术图。
- 自然场景的细节不是套建筑房间模板：每次生成还会按 Seed 增加不规则天然掩体、地形障碍和危险点；`terrain/ledge/platform/ice-island` 图元在自身顶面生成局部网格，使格线与高低差一起走。
