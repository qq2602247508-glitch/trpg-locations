...
- `dungeon`：以房间图为先，按墓穴/神殿/矿井/监狱/龙巢/下水道/奥术风格生成多层房间、环路、秘密房间、首领区和垂直路线。
- CoC/现实机构不再共用单一走廊：医院采用交叉病房翼与临床核心，天文馆采用中央穹顶和径向展厅，博物馆采用庭院四翼与环形展线，警局采用递进安保链，学校采用 U 形教学翼，酒店采用公共首层与重复客房层；未知建筑才进入功能区域编译器。
