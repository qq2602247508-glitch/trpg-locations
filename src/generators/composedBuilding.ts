...
function programFor(context: GeneratorContext): BuildingProgram {
  ...
  if (has(text, ["庄园", "宅邸", "manor", "mansion", "estate", "villa"])) {
    const rooms: ProgramRoom[] = [
      ...
      { ...room("crypt", "Family burial crypt", 2, 26, 25, 8, 6, "combat", ["underground", "secret"]), absoluteElevationFeet: -11 },
    ];
  ...
  if (has(text, ["堡垒", "要塞", "fortress", "citadel", "bastion"])) {
    const rooms = [
      ...
      room("inner-keep", "Inner keep hall", 0, 23, 29, 15, 10, "private", ["keep"]), { ...room("armory", "Underground armory", 1, 30, 28, 9, 7, "service", ["underground", "armory"]), absoluteElevationFeet: -12 },
