...
function inferDomain(text: string, requestedKind: SceneKind): { domain: SceneDomain; primaryKind: Exclude<SceneKind, "adaptive"> } {
  ...
  if (has(text, ["城市", "城镇", "水镇", "村镇", "村庄", "村落", "灯塔村", "海岸村", "海崖村", "渔村", "街区", "港区", "港镇", "深水城", "水城", "运河城", "聚落", "city", "town", "water town", "tidal town", "village", "district", "harbor", "port town", "water city", "canal city", "settlement"])) return { domain: "settlement", primaryKind: "settlement" };
...
function inferOperators(text: string): { morphology: MorphologyOperator[]; coverage: CoverageOperator[] } {
  ...
  if (has(text, ["河流", "河谷", "溪流", "小溪", "溪边", "溪畔", "林溪", "河道", "水城", "运河", "支流", "river", "stream", "creek", "streamside", "creekside", "canal", "water city"])) morphology.push("channel-cut");
  ...
  if (has(text, ["森林", "树林", "雨林", "丛林", "forest", "woodland", "rainforest", "jungle"])) coverage.push("woodland", "dense");
